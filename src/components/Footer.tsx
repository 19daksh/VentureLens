...
export const Footer: React.FC = () => {
  return (
    <footer className="bg-slate-900 dark:bg-slate-950 text-slate-300 dark:text-slate-400 border-t border-slate-800 dark:border-slate-900 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-12">
          ...
          {/* Product Links */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-200 dark:text-slate-300 mb-3">Product</h4>
            <ul className="space-y-2 text-xs text-slate-400 dark:text-slate-400">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-200 dark:text-slate-300 mb-3">Validation Core</h4>
            <ul className="space-y-2 text-xs text-slate-400 dark:text-slate-400">
          ...
          {/* Security & Access */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-200 dark:text-slate-300 mb-3">Security & Trust</h4>
            <ul className="space-y-2 text-xs text-slate-400 dark:text-slate-400">
        ...
        <div className="mt-12 pt-8 border-t border-slate-800 dark:border-slate-850 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 dark:text-slate-500 gap-4">
          <p>© {new Date().getFullYear()} VentureLens AI. All rights reserved. Validate your idea before you build it.</p>
          <div className="flex items-center gap-6">
            <Link to="/about" className="hover:text-slate-400 dark:hover:text-slate-300">About</Link>
            <Link to="/pricing" className="hover:text-slate-400 dark:hover:text-slate-300">Terms</Link>
            <Link to="/how-it-works" className="hover:text-slate-400 dark:hover:text-slate-300">Methodology</Link>
          </div>
        </div>
      </div>
    </footer>
