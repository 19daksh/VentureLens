...
export const DashboardPage: React.FC = () => {
  ...
  const { ideas, loading, deleteIdea, selectedCompareIds, toggleCompareId, clearCompare } = useAnalysis();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 my-8">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
            ...
            <p className="text-3xl font-extrabold text-slate-900 mt-2">{metrics.total}</p>
            <p className="text-[11px] text-slate-500 mt-1">Startup concepts tested</p>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
            ...
            <p className="text-3xl font-extrabold text-indigo-600 mt-2">{metrics.avgScore}<span className="text-sm font-semibold text-slate-400">/100</span></p>
            <p className="text-[11px] text-slate-500 mt-1">Portfolio viability index</p>
          </div>

          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Strong Signals</span>
              <Award className="w-4 h-4 text-emerald-500" />
            </div>
            <p className="text-3xl font-extrabold text-emerald-600 mt-2">{metrics.strongBuilds}</p>
            <p className="text-[11px] text-slate-500 mt-1">Score ≥ 80 (High conviction)</p>
          </div>

          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Pivots Advised</span>
              <AlertTriangle className="w-4 h-4 text-amber-500" />
            </div>
            <p className="text-3xl font-extrabold text-amber-600 mt-2">{metrics.pivots}</p>
            <p className="text-[11px] text-slate-500 mt-1">Score &lt; 60 (Requires pivot)</p>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs mb-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-200 text-xs focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <select
                ...
                className="text-xs rounded-lg border border-slate-200 py-2 px-2.5 bg-white text-slate-700"
              >
        ...
        {loading ? (
          <div className="bg-white rounded-xl border border-slate-200 p-16 text-center shadow-xs">
            ...
            <p className="text-xs font-semibold text-slate-600">Loading your startup validations...</p>
          </div>
            ...
            {filteredIdeas.map(item => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl border transition-all p-6 shadow-xs hover:shadow-md cursor-pointer flex flex-col justify-between group ${
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <span className="text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full truncate max-w-[160px]">
                        {item.industry}
                      </span>
                      {score !== undefined ? (
