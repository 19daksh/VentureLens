      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-xs mb-8 transition-colors">
          <div className="flex items-center gap-4 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div className="w-14 h-14 rounded-full bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold text-xl border-2 border-indigo-200 dark:border-indigo-800">
              {profile?.full_name?.charAt(0) || user?.email?.charAt(0).toUpperCase() || 'F'}
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{profile?.full_name || 'Founder'}</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">{user?.email}</p>
              <div className="mt-1 flex items-center gap-2">
                <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-900/60 px-2 py-0.5 rounded-full">
                  Active Founder Plan
                </span>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4 mt-6">
            ...
            <div>
              ...
              <input
                ...
                disabled
                value={user?.email || ''}
                className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 cursor-not-allowed"
              />
            </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-xs transition-colors">
          <h3 className="text-base font-bold text-slate-900 dark:text-white pb-4 border-b border-slate-100 dark:border-slate-800">
            Security & Password
          </h3>
          ...
          <form onSubmit={handleUpdatePassword} className="space-y-4 mt-6">
            ...
            <div className="pt-2">
              <button
                ...
                className="bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-700 text-white font-bold px-4 py-2 rounded-lg text-xs shadow-xs transition-colors disabled:opacity-50 flex items-center gap-2"
              >
